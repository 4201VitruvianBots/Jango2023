...
public class FieldSim {
    ...
    public FieldSim(DriveTrain driveTrain, Turret turret) {
    ...
    private void updateIntakePoses() {
        ...
        Pose2d robotPose = m_driveTrain.getRobotPose2d();
        double robotX = robotPose.getX();
        double robotY = robotPose.getY();
        double cos = robotPose.getRotation().getCos();
        double sin = robotPose.getRotation().getSin();

        double deltaXa = (robotX - SimConstants.robotLengthMeters / 2.0 - SimConstants.intakeLengthMeters) - robotX;
        double deltaXb = (robotX - SimConstants.robotLengthMeters / 2.0) - robotX;
        double deltaYa = (robotY + SimConstants.robotWidthMeters / 2.0 ) - robotY;
        double deltaYb = (robotY - SimConstants.robotWidthMeters / 2.0 ) - robotY;
...
//            resetRobotPose(new Pose2d(SimConstants.fieldWidth / 2.0 ,SimConstants.fieldHieght / 2.0 , new Rotation2d(0)));
        ...
        m_field2d.setRobotPose(m_driveTrain.getRobotPose2d());

        m_field2d.getObject("Turret").setPose(new Pose2d(m_driveTrain.getRobotPose2d().getTranslation(),
                new Rotation2d(Math.toRadians(getIdealTurretAngleDegrees()))));
    ...
    private void updateBallState(Powercell powercell) {
        ...
        if(powercell.getBallState() != 1) {
            if (ballPose.getX() < 0 || ballPose.getX() > SimConstants.fieldWidthMeters || ballPose.getY() < 0 || ballPose.getY() > SimConstants.fieldHieghtMeters)
...
//        System.out.println("Ball State: " + ballState + "\tCos: " + ballPose.getRotation().getCos() + "\tX Pos: " + ballPose.getX());
        switch (powercell.getBallState()) {
            case 3:
                ...
                if(ballPose.getX() < SimConstants.fieldWidthMeters / 2.0)
            ...
            case 2:
                ...
                double deltaX = powercell.getBallVelocity().getX() * deltaT;
                double deltaY = powercell.getBallVelocity().getY() * deltaT;
